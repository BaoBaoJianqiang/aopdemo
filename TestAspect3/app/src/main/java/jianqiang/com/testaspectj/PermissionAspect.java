...
public class PermissionAspect {
    ...
    private static final String POINTCUT_METHOD_PERMISS_CHECK = "execution(@jianqiang.com.testaspectj.PermissionCheck * *testAAA(..)) && @annotation(permissionCheck)";//所有方法已TakePhoto结尾的方法，这里的@annotation(permissionCheck) ，里面一定是首字母小写，否则失效
        ...
        if (permissionCheck.permession() != null && permissionCheck.permession().length != 0) {
            for (int i = 0; i < permissionCheck.permession().length; i++) {
                if (ContextCompat.checkSelfPermission((Context) joinPoint.getTarget(),
                        permissionCheck.permession()[i]) != PackageManager.PERMISSION_GRANTED) {
                    Log.d(className, className + ":" + methodName + "没有" + permissionCheck.permession()[i] + "的权限执行");
                }
            }
        }
    ...
    public void before(JoinPoint joinPoint, PermissionCheck permissionCheck) throws Throwable {
        ...
        Log.d(className, className + ":" + methodName + "on before");
        ...
        if (permissionCheck.permession() == null || permissionCheck.permession().length == 0) {
            ...
            for (int i = 0; i < permissionCheck.permession().length; i++) {
                Log.d(className, "检查" + permissionCheck.permession()[i] + "权限，in before ");
                if (joinPoint.getTarget() instanceof Activity) {
                    if (ContextCompat.checkSelfPermission((Context) joinPoint.getTarget(),
                            permissionCheck.permession()[i])
                            != PackageManager.PERMISSION_GRANTED) {
                        Log.d(className, "没有" + permissionCheck.permession()[i] + "权限，正在申请权限 in before");
                        needRequestPermissions = true;
                    } else {
                        Log.d(className, "已经有" + permissionCheck.permession()[i] + "权限， in before");
    ...
    public void after(JoinPoint joinPoint, PermissionCheck permissionCheck) throws Throwable {
        ...
        String className = methodSignature.getDeclaringType().getSimpleName();
        String methodName = methodSignature.getName();
        Log.d(className, className + ":" + methodName + "on after");
    }
